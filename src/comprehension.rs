...
#[cfg(test)]
mod hashmap_comprehension_tests {
    ...
    fn hashmap_comprehension() {
        ...
        let map = m!{key => key.to_uppercase(), for key in v};
    ...
    fn hashmap_comprehension_two() {
        let v = vec!["one", "two", "three"];
        let map = m!{format!("{}-true", key) => key.to_uppercase(), for key in v};
    ...
    fn hashmap_comprehension_three() {
        ...
        let map = m!{key => val, for (key, val) in v};
    ...
    fn conditional_hashmap_tuple_comprehension() {
        let v: Vec<(&str, i32)> = vec![("one", 1), ("two", 2), ("three", 3)];
        let map = m!{key => val, for (key, val) in v, if val == 1 || val == 2};
    ...
    fn hashmap_from_iter() {
        let map = m!{*key => key*key, for key in vec![1,2].iter()};
    ...
    fn conditional_hashmap_from_iter() {
        let map = m!{*key => key*key, for key in vec![1,2].iter(), if *key % 2 == 0};
        let mut e: HashMap<i32, i32> = HashMap::new();
        e.insert(2, 4);
    ...
    fn hashmap_from_range() {
        let map = m!{key => key*key, for key in 1..3};
        let mut e: HashMap<i32, i32> = HashMap::new();
        e.insert(1, 1);
        e.insert(2, 4);
    ...
    fn conditional_hashmap_from_range() {
        let map = m!{key => key*key, for key in 1..6, if key % 2 == 1};
        let mut e: HashMap<i32, i32> = HashMap::new();
        e.insert(1, 1);
        e.insert(3, 9);
        e.insert(5, 25);
